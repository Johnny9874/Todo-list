...
class TaskService {
    ...
    public function addTask($title, $description, $userId, $priority, $status, $due_date, $task_data) {
        ...
        // Récupérer la tâche ajoutée par son ID
        $taskId = $stmt->insert_id;  // Obtenir l'ID de la tâche insérée
        $task = $this->taskDAO->getTaskById($taskId);  // Utiliser TaskDAO pour récupérer la tâche
    
        if ($task === null) {
            throw new Exception("La tâche ajoutée n'a pas pu être récupérée.");
        }
    
        $stmt->close();
        
        return $task; // Renvoyer la tâche ajoutée
    }
